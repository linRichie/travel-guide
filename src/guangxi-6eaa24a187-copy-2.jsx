...
const GuangxiTravel = () => {
  ...
  const routePlans = {
    0: [ // 方案一：桂林-阳朔-龙脊-北海-涠洲岛
      { name: '桂林两江国际机场', location: [110.039317, 25.218618] },
      { name: '象鼻山', location: [110.264105, 25.268153] },
      { name: '阳朔西街', location: [110.496593, 24.778651] },
      // ... 其他景点
    ],
    1: [ // 方案二：南宁-德天瀑布-北海
      { name: '南宁吴圩国际机场', location: [108.171125, 22.608415] },
      { name: '青秀山', location: [108.386796, 22.787001] },
      { name: '德天瀑布', location: [106.644554, 22.704771] },
      { name: '通灵大峡谷', location: [106.938238, 24.611939] },
      { name: '北海银滩', location: [109.119254, 21.473343] },
      { name: '北海老街', location: [109.107185, 21.481116] }
    ],
    2: [ // 方案三：桂林-黄姚古镇-梧州-南宁
      { name: '桂林两江国际机场', location: [110.039317, 25.218618] },
      { name: '象鼻山', location: [110.264105, 25.268153] },
      { name: '黄姚古镇', location: [111.276527, 24.421111] },
      { name: '梧州骑楼城', location: [111.305473, 23.485038] },
      { name: '南宁青秀山', location: [108.386796, 22.787001] },
      { name: '南宁民歌湖', location: [108.393168, 22.816226] }
    ]
  };
  ...
  useEffect(() => {
    AMapLoader.load({
      ...
      version: '2.0',
      plugins: ['AMap.Scale', 'AMap.ToolBar', 'AMap.Driving'] // 添加 Driving 插件
    }).then((AMap) => {
      // 创建地图实例
      mapInstance.current = new AMap.Map(mapContainerRef.current, {
        ...
        mapStyle: 'amap://styles/whitesmoke' // 使用类似 Google Maps 的浅色主题
      });
      ...
      drivingInstance.current = new AMap.Driving({
        ...
        panel: "panel", // 路线详情面板，这里可以设置为 null
        hideMarkers: false,
        showTraffic: true,
  ...
  // 更新路线的函数
  const updateRoute = async (tabIndex) => {
    if (!drivingInstance.current) return;

    const points = routePlans[tabIndex];
    ...
    const waypoints = points.slice(1, -1).map(point => 
      new AMap.LngLat(point.location[0], point.location[1])
    );

    // 起点和终点
    const start = new AMap.LngLat(points[0].location[0], points[0].location[1]);
    const end = new AMap.LngLat(points[points.length - 1].location[0], points[points.length - 1].location[1]);
    ...
    // 规划驾车导航路线
    drivingInstance.current.search(start, end, {
      waypoints: waypoints
    }, (status, result) => {
      if (status === 'complete') {
        // 路线规划成功，自动调整地图视野以显示所有路线
